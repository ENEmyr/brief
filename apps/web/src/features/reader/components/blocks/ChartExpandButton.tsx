 ...
 */
export function ChartExpandButton({ chartRef, palette }: ChartExpandButtonProps) {
  const { open } = useDiagramViewer()

  const onExpand = useCallback(() => {
    const chart = chartRef.current
    if (!chart) return
    const dataUrl = chart.getDataURL({ pixelRatio: 2, backgroundColor: palette.card })
    open(`<img src="${dataUrl}" alt="chart" />`)
  }, [chartRef, palette, open])
