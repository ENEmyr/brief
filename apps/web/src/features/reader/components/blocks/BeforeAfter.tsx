...
function segmentClass(active: boolean): string {
  const base = 'rounded-md px-3 py-1 text-[12px] font-semibold'
    ...
    <div className="my-4 overflow-hidden rounded-xl border border-line bg-card">
      <div className="flex items-center justify-between border-b border-line2 bg-elev px-3.5 py-[9px]">
        ...
        <div className="flex items-center gap-0.5 rounded-lg bg-chip p-0.5">
          <button type="button" onClick={() => setShowAfter(false)} className={segmentClass(!showAfter)}>
            Before
          </button>
          <button type="button" onClick={() => setShowAfter(true)} className={segmentClass(showAfter)}>
            After
          </button>
        </div>
      </div>
